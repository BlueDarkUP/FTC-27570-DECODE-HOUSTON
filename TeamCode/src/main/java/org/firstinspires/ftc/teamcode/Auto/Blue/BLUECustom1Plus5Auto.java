...
@Autonomous(name = "蓝方远点1+5自动", group = "Autonomous")
public class BLUECustom1Plus5Auto extends OpMode {
    ...
    // 第三排路径
    public PathChain shootToPreIntake3; // (55,12) -> (40,34)
    public PathChain preIntake3ToCorner3; // (40,34) -> (10,34)
    public PathChain corner3ToShoot;      // (10,34) -> (55,12)
    ...
    private final Pose preIntake3Pose = new Pose(40.000, 34.000, Math.toRadians(180));
    private final Pose corner3Pose = new Pose(10.000, 34.000, Math.toRadians(180));
    private final Pose cornerAPose = new Pose(10.000, 10.000, Math.toRadians(180));
    private final Pose cornerBPose = new Pose(12.000, 34.000, Math.toRadians(180));
    ...
    public void buildPaths() {
        ...
        // A/B 循环路径
        shootToCornerA = follower.pathBuilder().addPath(new BezierLine(shootPose, cornerAPose)).setConstantHeadingInterpolation(Math.toRadians(180)).build();
        cornerAToShoot = follower.pathBuilder().addPath(new BezierLine(cornerAPose, shootPose)).setConstantHeadingInterpolation(Math.toRadians(180)).build();
        shootToCornerB = follower.pathBuilder().addPath(new BezierLine(shootPose, cornerBPose)).setConstantHeadingInterpolation(Math.toRadians(180)).build();
        cornerBToShoot = follower.pathBuilder().addPath(new BezierLine(cornerBPose, shootPose)).setConstantHeadingInterpolation(Math.toRadians(180)).build();
        ...
        switch (pathState) {
            ...
            case 31:
                if (!follower.isBusy()) {
                    ...
                    // 到达准备点末尾，立即打开 Intake 并前往吸取点 (10, 34)
            ...
            case 33:
                ...
                if (pathTimer.getElapsedTimeSeconds() >= INTAKE_WAIT_TIME) {
                    intakeShooter.setIntakePower(0.0); // <-- 修改点：在返回前提前关闭 Intake
